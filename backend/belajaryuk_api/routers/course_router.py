...
def create_course(
    *, 
    ...
    from belajaryuk_api.utils.input_validator import InputValidator
    
    is_valid, validation_errors = InputValidator.validate_course_input(
        topic=course_in.topic,
        goal=course_in.goal or "",
        description=""  # Description is generated by AI, not user input
    )
    
    if not is_valid:
        # Join all validation errors into a single user-friendly message
    ...
    # Sanitize the inputs
    course_in.topic = InputValidator.sanitize_input(course_in.topic)
    if course_in.goal:
        course_in.goal = InputValidator.sanitize_input(course_in.goal)
